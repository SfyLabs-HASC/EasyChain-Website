...
                <td><Link to={`/gestisci/${batch.batchId.toString()}`} className="web3-button">Gestisci</Link></td>
            </tr>
...
                    {/* --- LA CORREZIONE CHIAVE È QUI --- */}
                    <div className="card-footer"><Link to={`/gestisci/${batch.batchId.toString()}`} className="web3-button">Gestisci</Link></div>
                </td>
            </tr>
            {showDescription && (
...
            const formattedBatches = events.map((event: any): BatchData | null => {
...
    };
    ...
    // Effetto per caricare i dati quando l'account è disponibile
    useEffect(() => {
        if (account?.address) {
            fetchAllBatches();
        }
    ...
    // Effetto per filtrare i lotti quando i filtri o i dati cambiano
    useEffect(() => {
...
    const handleCloseModal = () => setModal(null);
    ...
    if (!contributorData[2]) { // Se il contributor non è attivo
        return <RegistrationForm />; 
    }

    // Se tutto è caricato e l'utente è attivo, mostra la dashboard
    return (
        <>
                ...
                <div className="modal-overlay" onClick={handleCloseModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-footer" style={{ justifyContent: 'space-between' }}>
                            ...
                            <button onClick={handleInitializeBatch} className="web3-button" disabled={isPending}>Crea</button>
                        </div>
                    </div>
                </div> 
            )}
            {(isPending || loadingMessage) && <TransactionStatusModal status={'loading'} message={loadingMessage || "In corso..."} onClose={() => {}} />}
...
            <main className="main-content-full">
                {/* Se l'utente non è connesso, mostra il pulsante di login al centro */}
                {!account ? (
                    ...
                    // Se l'utente è connesso, renderizza il componente Dashboard
                    <Dashboard />
                )}
            </main>
        </div>
